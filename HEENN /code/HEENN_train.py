...
def train(net, train_iter, num_epochs, lr, wd, X_test, y_test, fold=0, device=torch.device(f'cuda:1')):
    net.to(device)
...
def train_KFold(lr, wd, KG1, data, n_splits, num_epochs, batch_size):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(features, labels)):
        ...
        # X_train, y_train 列合并
        KG2 = torch.tensor(pd.concat([X_train, y_train], axis=1).to_numpy()).to('cuda:1')
